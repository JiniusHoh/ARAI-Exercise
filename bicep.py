...
def video_transform(frame):
  # You can add image processing logic here (optional)
  frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
  return frame

st.title("Webcam Streamlit App")

webrtc_streamer = st.webrtc_streamer(key="video", video_transform_func=video_transform)

if webrtc_streamer.media_stream:
  frame = webrtc_streamer.video_frames[0]
  st.image(frame)
